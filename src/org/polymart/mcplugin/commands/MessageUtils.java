...
public class MessageUtils{
    ...
    public static boolean verifyAccount(CommandSender sender){
        if(!PolymartAccount.hasToken()){
            if(PolymartAccount.config.getString("unlinkReason") != null && PolymartAccount.config.getString("unlinkReason").equalsIgnoreCase("IP_BAD")){
                sendMessage(sender, "It looks like your IP address changed since the last time you used the Polymart plugin! To log back in, use " + ChatColor.YELLOW + "/polymart login");
            }
            else{
                sendMessage(sender, "You aren't logged in! Log in with " + ChatColor.YELLOW + "/polymart login");
            }
            return false;
        }
        return true;
