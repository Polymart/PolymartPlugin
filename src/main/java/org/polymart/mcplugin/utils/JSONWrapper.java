...
public class JSONWrapper {
  ...
  public JSONWrapper(String value) {
    ...
    try {
      this.json = (JsonObject) new JsonParser().parse(value);
    } catch (Exception ex) {
      this.json = null;
    }
  }
  ...
  public JSONWrapper(JsonObject unwrapped) {
    this.json = unwrapped;
  }

  public JSONWrapper(JsonArray unwrapped) {
    this.list = unwrapped;
  }
  ...
  public JSONWrapper get(String key) {
    if (this.json != null) {
      ...
      clone.json =
          this.json.has(key) && clone.value.isJsonObject() ? clone.value.getAsJsonObject() : null;
  ...
  public JSONWrapper get(int index) {
    if (this.list != null) {
      ...
      if (obj instanceof JsonObject) {
        clone.json = (JsonObject) obj;
      }
      if (obj instanceof JsonArray) {
        clone.list = (JsonArray) obj;
      }
    ...
    if (array != null) {
      return new JSONWrapper(array.get(index));
    } else {
      return new JSONWrapper();
    }
  }
  ...
  public List<JSONWrapper> iterator() {
    ...
    for (Object obj : this.list == null ? new ArrayList<>() : this.list) {
      if (obj instanceof JsonObject) {
        list.add(new JSONWrapper((JsonObject) obj));
      } else if (obj instanceof JsonArray) {
        list.add(new JSONWrapper((JsonArray) obj));
      }
  ...
  public String asString(String defaultValue) {
    if (!isNull() && this.value.getAsString() != null) {
      return this.value.getAsString();
    }
    if (this.asInteger() != null) {
      return Objects.toString(this.asInteger());
    }
    return defaultValue;
  }
  ...
  public Float asFloat(Float defaultValue) {
    if (!isNull()) {
      return (Float) this.value.getAsFloat();
    }
    return defaultValue;
  }

  public List<String> asStringList() {
    return this.asStringList(null);
  }
