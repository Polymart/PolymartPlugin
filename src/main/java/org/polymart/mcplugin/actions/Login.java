...
public class Login {
  ...
  public static void login(CommandSender sender, String[] args) {
    if (PolymartAccount.hasToken()) {
      sendMessage(
          sender,
          "It looks like you're already logged in as https://polymart.org/user/"
              + PolymartAccount.getUserID()
              + ". If you want to switch accounts, use /polymart logout");
      return;
    }
        ...
        (JSONWrapper json) -> {
          ...
          if (json.get("success").asBoolean(false)) {
            ...
            if (sender instanceof ConsoleCommandSender) {
              ...
              sendMessage(
                  sender,
                  "Click here to finish linking your Polymart Account: " + ChatColor.YELLOW + url);

              //                    Bukkit.getServer().dispatchCommand(Bukkit.getConsoleSender(),
              //                            "tellraw " + sender.getName() +
              //                                    " {text:\"Click Here to finish linking your
              // Polymart Account\",clickEvent:{action:open_url,value:\"" +
              //                                    url + "\"}}"
              //                    );
            }

            PolymartAccount.checkForLink(
                token,
                (Object[] info) -> {
                  if (info != null) {
                    sendMessage(
                        sender,
                        "Awesome! Your Polymart account has been successfully linked. Your Polymart account: https://polymart.org/user/"
                            + info[0]);
            ...
            sendMessage(
                sender,
                "Uh oh! It looks like something went wrong. Are you sure your server is connected to the internet and can access https://api.polymart.org? Check the console for errors.");
          }
        });
  }
}
