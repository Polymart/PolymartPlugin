    ...
    PolymartAPIHandler.post(
        ...
        (JSONWrapper wrapper) -> {
          if (wrapper == null || wrapper.get("result").get("message").asString("").isEmpty()) {
            sendMessage(
                sender,
                "It looks like we weren't able to communicate with Polymart to log you out. Are you sure your server is connected to the internet and can access https://api.polymart.org?");
          } else {
