...
public class PolymartCommand implements TabExecutor {
  ...
  public static final List<String> TABCOMPLETE_ARGUMENTS =
      Arrays.asList("help", "search", "account", "login", "logout", "version", "update", "server");
  private static final List<String> alreadyLinkedElswhere = new ArrayList<>();
    ...
    if (cmd.getName().equalsIgnoreCase("polymart")) {
      if (args.length == 0 || args[0].toLowerCase().matches("^(web|website|info|version|ver)$")) {
        sendMessage(
            sender,
            "Running Polymart v"
                + Main.that.getDescription().getVersion()
                + " for https://polymart.org");
        ...
        if (verifyPermission(sender, "polymart.admin")) {
          if (verifyAccount(sender)) {
            sendMessage(
                sender,
                "You're currently logged in to Polymart as "
                    + ChatColor.YELLOW
                    + "https://polymart.org/user/"
                    + PolymartAccount.getUserID()
                    + ChatColor.WHITE
                    + ". To log out, use /polymart logout");
          }
        }
        ...
        if (verifyPermission(sender, "polymart.admin")) {
          if (!PolymartAccount.hasToken()) {
            ...
            sendMessage(
                sender,
                "Are you sure you want to log out? To confirm, type /polymart logout. This will expire in 10 seconds.");
          }
        }
        ...
        if (verifyPermission(sender, "polymart.admin") && verifyAccount(sender)) {
          if (!PolymartAccount.serverLinked()) {
            if (linkingServer
                ...
                && args[2].length() > 0) {
              if (alreadyLinkedElswhere.contains(args[2])
                  && (args.length < 4 || !args[3].equalsIgnoreCase("overwrite"))) {
                sendErrorMessage(
                    sender,
                    "This listing is already linked with another server. Linking it here will remove it from your other server. Are you sure you want to do this? To confirm, type "
                        + ChatColor.YELLOW
                        + "/polymart server link "
                        + args[2]
                        + " overwrite");
              } else {
                sendMessage(
                    sender,
                    "Linking this server with your Polymart listing (polymart.org/server/"
                        + args[2]
                        + ")...");
                ...
                PolymartAPIHandler.post(
                    ...
                    (JSONWrapper json) -> {
                      if (json.get("success").asBoolean(false)) {
                        ...
                        sendMessage(
                            sender,
                            "Success! This server has been linked with your Polymart listing "
                                + ChatColor.GREEN
                                + "polymart.org/server/"
                                + args[2]);
                        ...
                        sendErrorMessage(
                            sender,
                            "It looks like something went wrong trying to link the server!");
                      }
                    });
                linkingServer = false;
              }
            ...
            sendMessage(
                sender,
                "Let's link your server with Polymart! Loading the servers you have listed on Polymart...");
            PolymartAPIHandler.post(
                "getUserServers",
                new HashMap<>(),
                (JSONWrapper json) -> {
                  ...
                  if (l == null || l.size() < 1) {
                    sendMessage(
                        sender,
                        "It looks like you haven't listed any servers on Polymart yet. Let's start with that! To list add your server, go to "
                            + ChatColor.BLUE
                            + "https://polymart.org/server/new"
                            + ChatColor.AQUA
                            + ". When you've added your server, come back here and type "
                            + ChatColor.BLUE
                            + "/polymart server");
                  } else {
                    ...
                    sendMessage(
                        sender,
                        "Link your server to improve your experience on Polymart, access new features on your server listing, and to get more players! Polymart will periodically be sent info like the online players, the plugins you use, and other info and stats about your server.");
                    sendMessage(
                        sender,
                        "You have the following servers listed on your Polymart account."
                            + (player
                                ? "Click the listing that you'd like to link this server with:"
                                : ""));
                    ...
                    for (JSONWrapper w : l) {
                      ...
                      if (player) {
                        f.appendClickableWithCommand(
                            str, "polymart server link " + w.get("id").asString());
                      } else {
                        f.append(
                            str
                                + " "
                                + ChatColor.GRAY
                                + "- link with /polymart server link "
                                + w.get("id").asString());
                      }
                      f.append(ChatColor.GRAY + "\n");
                    }
            ...
            sendMessage(
                sender,
                "This server is linked with Polymart at "
                    + ChatColor.BLUE
                    + "https://polymart.org/server/"
                    + PolymartAccount.getServerID());
          }
        }
      }
