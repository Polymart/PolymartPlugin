      ...
      if (args.length == 1 && args[0].toLowerCase().matches("^(help|\\?)$")) {
        sendMessage(
            ...
            "Use "
                ...
                + "/update"
                + ChatColor.WHITE
                + " to see a list of plugins with updates available");
        sendMessage(
            sender,
            "Use " + ChatColor.AQUA + "/update all" + ChatColor.WHITE + " to update all plugins");
        sendMessage(
            sender,
            "Use "
                + ChatColor.AQUA
                + "/update FirstPluginName AnotherPluginName ..."
                + ChatColor.WHITE
                + " to update the plugins in the list");
        sendMessage(
            sender,
            "Use " + ChatColor.AQUA + "/update help" + ChatColor.WHITE + " to see this message");
        return true;
      }
      update(sender, args);
      return true;
